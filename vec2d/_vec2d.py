...
class vec2d(object):
    ...
    def __getitem__(self, key):
        if key == 0:
            ...
            return self.y
        else:raise IndexError("Invalid subscript "+str(key)+" to vec2d")
    ...
    def get_lengthed(self, length):
        l = math.sqrt(self.x*self.x + self.y*self.y) 
        try:return vec2d(self.x * length/l, self.y * length/l)
        except ZeroDivisionError:
            return vec2d(0,0)
        
    @staticmethod
    def create_rotation_vector(angle):
        """Generate a rotation vector from angle to be used for 'rotate_vector'"""
    ...
    @staticmethod
    def create_rotation_vector_radians(radians):
        """Generate a rotation vector from radians to be used for 'rotate_vector'"""
        cos = math.cos(radians)
        sin = math.sin(radians)
        return vec2d(cos, sin)
    ...
    def rotate_by_vector(self, rotation):
        """Rotates by another rotation vector, made by 'create_rotation_vector'"""
        self.x, self.y = self.x*rotation.x - self.y*rotation.y, self.x*rotation.y + self.y*rotation.x
    
    def rotate(self, angle_degrees):
        radians = math.radians(angle_degrees)
        cos = math.cos(radians)
        sin = math.sin(radians)
        tmp = self.x*cos - self.y*sin
        self.y = self.x*sin + self.y*cos
        self.x = tmp
        
    @staticmethod
    def fromangle1(angle_degrees):
        radians = math.radians(angle_degrees)
        cos = math.cos(radians)
        sin = math.sin(radians)
        return vec2d(cos, sin)
    ...
    @staticmethod
    def fromangle(angle_degrees, length):
        radians = math.radians(angle_degrees)
        cos = math.cos(radians)
        sin = math.sin(radians)
    ...
    def __setradians(self, angle_radians):
        self.x = self.length
        self.y = 0
        self.rotate_radians(angle_radians)
        
    radians = property(get_radians, __setradians, None, "gets or sets the radians of a vector")
